...
export function parseBook() {
  try {
    if (!fs.existsSync(BOOK_PATH)) {
      console.warn(`Book file not found at ${BOOK_PATH}, skipping parsing`);
      return;
    }
    ...
    if (firstHeaderIndex === -1) {
      console.warn('Could not find first ### header in index.md');
      return;
    }
    ...
    for (let i = firstHeaderIndex; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('## ')) {
        ...
        // Skip pushing part header lines to chapter markdown
        continue;
      }
      ...
      // Accumulate lines for current chapter
      if (currentChapter) {
...
`;
    ...
    console.log(`[Vite Build/Dev Parser] Generated ${chapters.length} chapters in ${OUTPUT_PATH}`);
  } catch (error) {
    console.error('Error during auto book parsing:', error);
  }
}
